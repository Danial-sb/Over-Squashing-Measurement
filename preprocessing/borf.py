...
class CurvaturePlainGraph():
    ...
    def _transport_plan_uv(self, u, v, method='OTD', u_neighbors=None, v_neighbors=None):
        u_neighbors = [p for p in range(self.V) if
                       self.adjacency_matrix[u][p] == 1] if u_neighbors is None else u_neighbors
        v_neighbors = [q for q in range(self.V) if
                       self.adjacency_matrix[v][q] == 1] if v_neighbors is None else v_neighbors
    ...
    def curvature_uv(self, u, v, method='OTD', u_neighbors=None, v_neighbors=None):
        optimal_cost, optimal_plan = self._transport_plan_uv(u, v, method, u_neighbors=u_neighbors,
...
def borf3(
    ...
    edge_index_filename = os.path.join(dirname,
                                       f'iters_{loops}_add_{batch_add}_remove_{batch_remove}_edge_index_{graph_index}.pt')
    edge_type_filename = os.path.join(dirname,
                                      f'iters_{loops}_add_{batch_add}_remove_{batch_remove}_edge_type_{graph_index}.pt')
    ...
    for _ in range(loops):
        ...
        if not _C:
            raise ValueError("No edges found in the graph for sorting!")
